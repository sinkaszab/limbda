...
test(
  'map | Map an empty Collection returns an empty Array:',
...
test(
  'map | Transforming function\'s return value:',
  function testTransformingFnsReturnVal(t) {
    t.plan(1);
    t.deepEquals(
      map(function intentionallyNoReturn() {
      }, [1, 2]), // eslint-disable-line prefer-arrow-callback
      [undefined, undefined], 'Function returns `undefined` by default.',
    );
  },
);

test(
  'map | Map a Collection with one element will return an Array with one mapped element:',
  function testCollectionWithOneElem(t) {
    t.plan(6);
    t.deepEquals( // 1
...
test(
  'map | Map different type of collections:',
  function testOnCollections(t) {
    ...
    t.deepEquals( // 5
      map(v => v + 1, new Set([1, 2, 2, 3, 4, 4, 4, 5, 6])).sort(compare),
      [2, 3, 4, 5, 6, 7], 'Set',
    );
    t.deepEquals( // 6
